 ...
 */
contract EthToInrPriceFeed is FunctionsClient, ConfirmedOwner, AutomationCompatibleInterface {
  ...
  address public constant priceFeedsEthToUsd = 0x694AA1769357215DE4FAC081bf1f309aDC325306;
  uint256 public priceAns;
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    (,int256 ethToUsd,,,) = AggregatorV3Interface(priceFeedsEthToUsd).latestRoundData();

    uint256 usdToInr = abi.decode(response, (uint256));

    priceAns = (uint256(ethToUsd) * usdToInr) / 1e8;

    latestResponse = abi.encode(priceAns);
